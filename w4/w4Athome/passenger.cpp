...
namespace holiday {
	...
	{
		if (!(name && *name) || !(destination && *destination) || !(year >= 2017 && year <= 2020) || !(month >= 1 && month <= 12) || !(day >= 1 && day <=31))
		{
			init();
		}
		else
	...
	void Passenger::travelWith(const Passenger* arrPassengers, int size)
	{
		...
		{
			if (!strcmp(m_destination, arrPassengers[i].m_destination) &&
				m_departureYear == arrPassengers[i].m_departureYear &&
