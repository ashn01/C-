...
namespace ict {
	...
	std::istream& Date::read(std::istream& is)
	{
		...
		if (is.get() == '/') // remove '/'
		{
			...
			if (is.get() == '/')
			{
				is >> d;
				format=true;
				if (is.get() == ','&& is.get() == ' ') // if , and ' ' then work
				{
					is >> h;
					if (is.get() == ':') // remove ':'
					{
					...
					is.putback('\n'); // at the end push '\n' in buffer because I guess is.get() read  last letter in buffer
									// so \n removed and skipped if statement. 
			}
		}
	...
	std::ostream& Date::write(std::ostream& ostr)const
	{
		char pre=ostr.fill();
		ostr.fill('0');
		ostr << m_year << "/" << setw(2)<< right << m_mon <<"/"<< setw(2)<<right<<m_day;
		/*if (m_mon < 10)  // inappropriate code
			ostr << "0"; // i could fix this set2(2), filled(0) etc..
		ostr << m_mon << "/";
		...
		if (!m_dateOnly)
		{
			ostr << ", "<<setw(2)<< right <<m_hour<<":"<<setw(2)<<right<<m_min;
			/*if (m_hour < 10)
				ostr << "0";
			ostr << m_hour << ":";
			if (m_min < 10)
